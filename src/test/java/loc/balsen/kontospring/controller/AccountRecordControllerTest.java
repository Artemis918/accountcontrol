...
public class AccountRecordControllerTest extends TestContext {
	...
	public void testLoadRecords() throws Exception {
		...
		mvc.perform(get("/accountrecord/unassigned")).andExpect(jsonPath("$.[*]", hasSize(startSize + 1)));

		createRecord("record1");
		mvc.perform(get("/accountrecord/unassigned")).andExpect(jsonPath("$.[*]", hasSize(startSize + 2)));
	...
	private void createAssignment(AccountRecord record) {
		...
		Plan plan = new Plan(0, null, null, null, null, 0, 1, new Pattern("\"sender\": \"help\""), "short",
				"description", null, subCategory1, null);
	...
	private AccountRecord createRecord(String description) {
		...
		AccountRecord result = new AccountRecord(0, null, null, executed, null, "sender", "receiver", 0, detlist,
